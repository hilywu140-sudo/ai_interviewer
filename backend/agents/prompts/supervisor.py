...
- 明确表达想要"练习"某个问题
- 提到"模拟面试"、"录音"等练习相关词汇
- 说"换一个问题"、"下一个问题"、"另外一个问题"
- 直接说出一个完整的面试问题(如"介绍一下你自己")
- 用户消息中包含具体的面试问题描述

**关键词**: 练习、模拟、录音、换、下一个、另外一个

**示例**:
- "我想练习自我介绍" → extracted_question: "请做一个简短的自我介绍"
- "换一个问题" → 需要从上下文推断下一个问题
- "我要练习介绍做过最困难的事情" → extracted_question: "请介绍一个你做过最困难的事情"
- "练习一下为什么离职这个问题" → extracted_question: "为什么离职"

### 2. answer_optimization(答案优化)→ chat
**触发条件**:
- 明确要求"优化"、"修改"、"改进"已有的回答内容
- 提到"刚才的回答"、"我的回答"
- 要求重写或润色答案

**关键词**: 优化、修改、改进、重写、润色、刚才的回答

**注意**: 历史信息中包含了用户录音的逐字稿,用户未说明的情况下,需要抓取历史消息中对应问题的逐字稿内容作为修改的基础文本
...
**关键词**: 怎么回答、如何回答、回答思路、怎么答、如何答、这个问题
...
**关键词**: 帮我写、写一个、写个、生成答案、给我一个回答、帮忙写、写逐字稿、起草
